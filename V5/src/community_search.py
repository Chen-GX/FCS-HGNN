...
def dgl_to_adj(g, graph_path): # g是dgl.graph
    ...
    print('start process search graph')
    for (src, dst) in tqdm(zip(*edge_index)):
...
class Community_Search(object):
    def __init__(self, args, graph_data, community_types, num_nodes, max_depth=-1):
        self.args = args
        if args.search_homo:
            ...
            if osp.exists(graph_path):
                ...
                self.graph = dgl_to_adj(graph_data.g, graph_path)
            self.before_num_node = 0  # 如果是全局搜，不需要减去
        # edges = 0
        # # 统计边数
        # for k, v in tqdm(self.graph.items()):
        #     edges += len(v)
        # print(edges)  # 42182144  去掉了自环
    ...
    def bfs(self, query, threshold, probs):
        ...
        while queue:
            ...
            if not visited[current_node] and (depth <= self.max_depth or self.max_depth == -1):
...
def search_threshold(lc, scorelists, num_data):
    ...
    f1_m = 0.0  # 最佳f1
    s_m = s_  # 最佳阈值
    best_len = 0  # 最佳结果的社区长度
    step = 0.05  # 增加步长
    thresholds = np.arange(s_, 0.85 + step, step)  # 计算所有阈值
    for s_ in tqdm(thresholds, desc=f'search threshold'):
        # logger.info(f"------------------------------ {s_}")
        com_len = []
        f1_x = 0.0
        for q, comm, probs in scorelists:
            comm_find_set = set(lc.bfs(q, s_, probs))
            com_len.append(len(comm_find_set))
            f1, _, _ = f1_score_(comm_find_set, set(comm))
            f1_x += f1
        ...
        if f1_m < f1_x:
            f1_m = f1_x
            s_m = s_
            best_len = np.mean(com_len)
    logger.info(f"------------------------ theshold {s_m} f1 {f1_m} len {best_len}")
    return s_m, f1_m, best_len
...
def search_threshold_parallel(lc, scorelists, num_data, max_processes=20):
    threshold_range = np.arange(0.05, 0.86, 0.05)
    ...
    with mp.Pool(processes=max_processes) as pool:
        ...
        results = list(tqdm(pool.imap(worker, [(s_, lc, scorelists, num_data) for s_ in threshold_range]),

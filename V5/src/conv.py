...
class myGATConv(nn.Module):
    ...
    def __init__(self,
        ...
        # 初始化BatchNorm层
        self.use_batchnorm = use_batchnorm  # TODO: cgx
        ...
        if use_batchnorm:  # TODO: cgx
            self.batch_norm_layer = nn.BatchNorm1d(self._num_heads)
            # self.batch_norm_layer = nn.ModuleList([nn.BatchNorm1d(self._out_feats) for _ in range(self._num_heads)])
    ...
    def forward(self, graph, feat, e_feat, res_attn=None):
        with graph.local_scope():
            ...
            if self.use_batchnorm:
                ...
                # rst = self.batch_norm_layer(rst.view(-1, self._out_feats)).view(-1, self._num_heads, self._out_feats)
                # for i in range(self._num_heads):
                #     # reshape data to [num_nodes, feature_dim] for each attention head
                #     rst[:, i, :] = self.batch_norm_layer[i](rst[:, i, :])

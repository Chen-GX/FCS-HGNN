...
def load_query(args, num_nodes, check=False):
    ...
    train_data = QueryDataset(args, data['primary'], data['train'], num_nodes)
    valid_data = QueryDataset(args, data['primary'], data['valid'], num_nodes)
    test_data = QueryDataset(args, data['primary'], data['test'], num_nodes)

    return data['primary'], train_data, valid_data, test_data




class QueryDataset(Dataset):
    def __init__(self, args, primary, data, num_nodes):
        """data 是 [[q, [com]], ...]"""
        super(QueryDataset, self).__init__()
        self.primary = primary
        self.num_nodes = num_nodes
        self.process(args, data)

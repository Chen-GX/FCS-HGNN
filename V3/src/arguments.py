...
def get_args():
    ...
    ## search
    parser.add_argument('--search_homo', type=str2bool, default=False)  # 是否用同构图进行搜索
    parser.add_argument('--before_num_node', type=int, default=0)
    parser.add_argument('--max_depth', type=int, default=-1)

    ## dataset 
    parser.add_argument('--data_dir', type=str, default='../../datasets_two_type')
    parser.add_argument('--data_name', type=str, default='Freebase')
    parser.add_argument('--query_name', type=str, default=None)
    
    parser.add_argument('--seed', type=int, default=42)
    parser.add_argument("--output_dir", type=str, default="../../../output_dir/V3/test")

    # parser.add_argument('--log', action='store_true', help='Whether to use wandb')
    args = parser.parse_args()  # 解析参数

    if args.data_name == 'DBLP':
        args.query_name = 'query_data_AP.json'
        args.community_types = [0, 1]
        args.before_num_node = 0

    elif args.data_name == 'ACM':
        args.query_name = 'query_data_PA.json'
        args.community_types = [0, 1]
        args.before_num_node = 0
    
    elif args.data_name == 'IMDB':
        args.query_name = 'query_data_MA.json'
        args.community_types = [0, 1]
        args.before_num_node = 0

    elif args.data_name == 'Freebase':
        args.query_name = 'query_data_SO.json'
        args.community_types = [0, 1]
        args.before_num_node = 0
    ...
    file_name = f"ep_{args.epoch}_lr_{args.lr}_bs_{args.batch_size}_{args.timestamp}"
    if args.search_homo:
